...
const SIZE = 460;
const CANVAS = 600;
const CANVAS_INSET = (CANVAS - SIZE) / 2;
const CENTER = SIZE / 2;
const RADIUS = 178;
  ...
  return (
    <div className="absolute" style={{ left: x, top: y, width: 0, height: 0 }}>
      <div className={cn("mkt-orbit-pill-upright", active && "mkt-orbit-pill-upright--on")}>
        <Link
          href={`#${mod.id}`}
          className="absolute left-0 top-0 flex w-[220px] -translate-x-1/2 -translate-y-1/2 items-center gap-3 rounded-2xl border border-mkt-navy/10 bg-white/95 px-4 py-3 shadow-lg shadow-mkt-navy/10 backdrop-blur-sm transition hover:border-mkt-navy/20 hover:shadow-xl"
        >
          <span className={cn("size-3 shrink-0 rounded-full", mod.dot)} />
          <span className="text-sm font-bold text-mkt-navy">{mod.label}</span>
          <span className="ml-auto font-mono text-[0.6rem] text-mkt-navy/35">
    ...
    >
      <div
        ...
        style={{
          width: SIZE,
          height: SIZE,
          marginLeft: CANVAS_INSET,
          marginTop: CANVAS_INSET,
          "--orbit-duration": `${ORBIT_DURATION}s`,
        }}
      >
        <div
          aria-hidden
          className="pointer-events-none absolute inset-4 rounded-full bg-gradient-to-br from-mkt-lime/18 via-mkt-sky/12 to-mkt-coral/12 blur-2xl"
        />
        ...
        <div className="relative z-[1] size-full overflow-visible">
          <svg viewBox={`0 0 ${SIZE} ${SIZE}`} className="pointer-events-none absolute inset-0 size-full" aria-hidden>
            ...
            {active ? (
              <g filter="url(#product-orbit-energy-glow)">
                <circle r={9} fill="url(#product-orbit-energy)" opacity={0.55}>
                  <animateMotion dur={`${ORBIT_DURATION}s`} repeatCount="indefinite" path={ORBIT_CIRCLE_PATH} calcMode="linear" />
                </circle>
                <circle r={4} fill="#fff">
                  <animateMotion dur={`${ORBIT_DURATION}s`} repeatCount="indefinite" path={ORBIT_CIRCLE_PATH} calcMode="linear" />
                </circle>
          ...
          <div
            className={cn(
              "mkt-orbit-carousel absolute inset-0 overflow-visible",
